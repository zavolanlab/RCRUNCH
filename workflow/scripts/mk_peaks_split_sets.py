...
#   randomly assigned in either the test or the training set (the two sets have
#   equal size).
...
def main():
    ...
    if len(peaks) > 1000:
        ...
        sys.stderr.write(
            'Peaks fewer than 20, so no point to continue with motif prediction.\
            Try using a higher FDR value.')
        sys.exit(1)
    ...
    training_set = open(os.path.join(
        options.out_folder,
        'training.fasta'), 'w')
    ...
    training_set_bg = open(os.path.join(
        options.out_folder,
        'training_bg.fasta'), 'w')
    ...
    for index, row in new_peaks.iterrows():
        ...
        for j in np.arange(1, 11):
            ...
            if index < len(new_peaks) / 2.0:
                ...
                training_set_bg.write(bg_line)
            peak_counter += 1
    test_set.close()
    test_set_bg.close()
    training_set.close()
    training_set_bg.close()
