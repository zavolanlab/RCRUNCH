...
def main():
    ...
    training_pool = os.path.join(initial_path, options.training_pool)
    training = os.path.join(initial_path, options.training)
    test_pool = os.path.join(initial_path, options.test_pool)

    phylogenetic_type = check_phylogenetic(training_pool)
    ...
    for wm in options.wms:
        ...
        parameters.append([wm, training_pool, training, test_pool,
...
def motevo_process(params):
    ...
    wm = params[0]
    training_pool = params[1]
    training = params[2]
    test_pool = params[3]
    genome_tag = params[4]
    phylogenetic_tree = params[5]
    outpath = params[6]
    ...
    if not each_logo:
        return pd.Series([])
    each_logo.fig.savefig(os.path.join(outpath, 'wlib', f'{wmname}.pdf'), transparent=True)
    plt.clf()
    # -----------------------------------------------------------
    ...
    # Calculate the priors for each of the motifs
    prior_report = 'training_prior_report'
    parameters_prior = 'training_prior_params'
    prior_sites = 'training_prior_sites'
    priors = 'priors'
    # make the motevo parameter file for the priors
    motevo_parameters_prior(
        parameters_prior, priors, prior_sites, genome_tag, phylogenetic_tree)
    prior_motevo = ['motevo', training, parameters_prior, wm]
    # call Motevo
    with open(prior_report, "wb") as outfile:
        # subprocess.call(prior_motevo, stdout=outfile)
        try:
            priorcall = Popen(prior_motevo, stdout=outfile, stderr=PIPE)
            priorcall.wait()
            stdout, stderr = priorcall.communicate()
        except CalledProcessError as err:
            print(f"Error ocurred: prior {wmname} {err}")
    priorpath = os.path.join(each_path, priors)
    bgprior = float(get_prior(priorpath))
    if bgprior == 1:
        bgprior = 0.99

    # ----------------------------------------------------------------
    # calculate motevo posteriors on training set - used for beta fitting
    posterior_report = os.path.join(each_path, 'training_posterior_report')
    parameters_posterior = os.path.join(each_path, 'training_posterior_params')
    posterior_sites = os.path.join(each_path, 'training_posterior_sites')
    posteriors = os.path.join(each_path, 'training_posteriors')
    ...
    motevo_parameters_posterior(bgprior, posteriors, posterior_sites,
                                ...
                                genome_tag, phylogenetic_tree)
    posterior_motevo = ['motevo', training_pool, parameters_posterior, wm]
    ...
    window_nr_training = get_sequence_length(training_pool, wm)
    beta = fit_beta(posterior_sites, window_nr_training)
    sys.stdout.write('{0} beta chosen {1}\n'.format(wmname, beta))
    sys.stdout.flush()
...
def motevo_parameters_prior(parampath, prior, sites, genome_tag,
                            phylogenetic_tree, ufewmprior=0, priordiff=0.0001,
                            ufewmfile=None, ufewmlen=8, ufeprint=0,
                            markovorderbg=1, bgprior=0.98,
